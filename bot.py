...
def get_categories_menu(action: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Категория 1", callback_data=f"{action}_cat_1")],
        [InlineKeyboardButton(text="Категория 2", callback_data=f"{action}_cat_2")],
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if CHANNEL_ID is None:
        await message.answer(
            ...
            "Отправьте команду /set_channel_id -100XXXXXXXXXX или перешлите сюда пост из нужного канала.")
...
async def category_selected(callback: CallbackQuery, state: FSMContext):
    ...
    if data.get("rating_mode"):
        ...
        text = f"🏆 **Топ-3 участниц (Категория {category}):**\n\n"
        ...
        await callback.message.edit_text(
            f"Вы выбрали Категорию {category}. Что хотите сделать?",
...
async def get_name_and_save(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            f"✅ Участница *{name}* добавлена в Категорию {category}.",
...
async def show_delete_page(callback: CallbackQuery, category: int, page: int):
    ...
    text = f"Выберите участницу для удаления (Категория {category}, стр. {page+1}/{pages}):"
    try:
...
async def back_to_category(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"Категория {category}. Что хотите сделать?",
        reply_markup=get_category_menu()
    )
    await callback.answer()
...
"""
        ...
        for category, count in category_stats:
            stats_text += f"• Категория {category}: {count} участниц\n"
        
        if top_participants:
            stats_text += "\n🏆 **Топ-5 участниц:**\n"
            for i, (name, votes, category) in enumerate(top_participants, 1):
                stats_text += f"{i}. {name} (Кат.{category}) - {votes} голосов\n"
